...
public class LichTapServiceImpl implements LichTapService {
    ...
    public LichTap createPTScheduleWithDate(String maNV, String maKH, String ngayTap, String caTap, String maKV) {
        try {
            ...
            while (!currentDate.isAfter(ngayKT)) {
                ...
                count++;
                System.out.println("✅ Đã tạo lịch tập " + count + ": " + savedLichTap.getMaLT() + " cho ngày "
                        + currentDate + " (" + thuValue + ")");
            ...
            System.out.println(
                    "🎉 Đã tạo tổng cộng " + count + " lịch tập lặp lại mỗi tuần (" + thuValue + ") trong thời hạn PT");
    ...
    @Override
    public LichTap createPTScheduleMultipleDays(String maNV, String maKH, String ngayBatDau, String thuString,
            String caTap, String maKV) {
        try {
            System.out.println("=== TẠO LỊCH PT VỚI NHIỀU NGÀY ===");
            System.out.println("Trainer: " + maNV);
            System.out.println("Khách hàng: " + maKH);
            ...
            // Kiểm tra ngày bắt đầu có nằm trong thời hạn PT không
            if (!isDateWithinPTPeriod(maKH, ngayBatDau)) {
                System.out.println("❌ Ngày bắt đầu không nằm trong thời hạn PT của khách hàng");
                return null;
            }

            // Lấy thông tin khách hàng và thời hạn PT
            KhachHang khachHang = khachHangRepository.findById(maKH).orElse(null);
            if (khachHang == null) {
                System.out.println("❌ Không tìm thấy khách hàng: " + maKH);
                return null;
            ...
            // Lưu vào database
            LichTap savedLichTap = lichTapRepository.save(lichTap);
            System.out.println("✅ Đã tạo lịch PT với các ngày: " + thuString);

            return savedLichTap;

        } catch (Exception e) {
            System.err.println("❌ Lỗi khi tạo lịch PT nhiều ngày: " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
    ...
    private boolean isDateWithinPTPeriod(String maKH, String ngayTap) {
        try {
            ...
            for (ChiTietDangKyDichVu pt : ptServices) {
                ...
                System.out.println(
                        "  - Trạng thái HĐ: " + (pt.getHoaDon() != null ? pt.getHoaDon().getTrangThai() : "null"));
                ...
                if (!selectedDate.isBefore(startDate) && !selectedDate.isAfter(endDate)) {
                    System.out.println("✅ Ngày " + selectedDate + " nằm trong khoảng thời gian PT: " + startDate + " - "
                            + endDate);
                    return true; // Ngày nằm trong khoảng thời gian PT
                } else {
                    System.out.println("⚠️ Ngày " + selectedDate + " KHÔNG nằm trong khoảng thời gian PT: " + startDate
                            + " - " + endDate);
                }
            }
    ...
    public LichTap createClassSchedule(String maNV, String maLop, String ngayTap, String caTap, String maKV) {
        try {
            ...
            if (dayOfWeek == 7) {
                ...
                thuValue = String.valueOf(dayOfWeek + 1);
            }
            ...
            System.err.println("❌ Lỗi khi tạo lịch Lớp: " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
