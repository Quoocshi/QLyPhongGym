...
public class LopServiceImpl implements LopService {
    ...
    public String getCaTapStringForLop(String maLop) {
        try {
            ...
            System.out.println("Debug: Tổng số lịch tập cho lớp " + maLop + ": " + (allLichTap != null ? allLichTap.size() : "null"));
            
            if (allLichTap != null && !allLichTap.isEmpty()) {
                for (hahaha.model.LichTap lt : allLichTap) {
                    System.out.println("Debug: LichTap - LoaiLich: " + lt.getLoaiLich() + ", Thu: " + lt.getThu() + ", CaTap: " + (lt.getCaTap() != null ? lt.getCaTap().getTenCa() : "null"));
                }
            }
            ...
            if (lichTapList == null || lichTapList.isEmpty()) {
                try {
                    ...
                    System.out.println("Debug: Thử tìm với LoaiLich='Lop': " + (lichTapList != null ? lichTapList.size() : "null"));
                } catch (Exception e) {
                ...
                for (int i = 0; i < lichTapList.size(); i++) {
                    ...
                    if (lichTap.getCaTap() != null) {
                        ...
                        if (tenCa != null && !tenCa.trim().isEmpty()) {
                            caTap = tenCa;
                            if (lichTap.getCaTap().getMoTa() != null && !lichTap.getCaTap().getMoTa().trim().isEmpty()) {
    ...
    public int getThoiHanLop(Lop lop) {
        try {
            if (lop.getNgayBD() != null && lop.getNgayKT() != null) {
                ...
                long daysBetween = ChronoUnit.DAYS.between(lop.getNgayBD().toLocalDate(), lop.getNgayKT().toLocalDate());
