...
public interface LichTapRepository extends JpaRepository<LichTap, String> {
       ...
       // Tìm lịch lớp theo trainer
       @Query("SELECT lt FROM LichTap lt WHERE lt.nhanVien.maNV = :maNV AND lt.loaiLich = 'Lop' ORDER BY lt.maLT")
       List<LichTap> findClassScheduleByTrainer(@Param("maNV") String maNV);
       ...
       Long countConflictingSchedules(@Param("maNV") String maNV, @Param("caTap") String caTap,
       ...
       @Query("SELECT COUNT(lt) FROM LichTap lt WHERE " +
                     ...
                     "AND lt.trangThai != 'Huy'")
       Long countConflictingSchedulesForDate(@Param("maNV") String maNV, @Param("caTap") String caTap,
                     @Param("ngayTap") String ngayTap);
