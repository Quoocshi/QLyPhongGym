...
public interface ChiTietDangKyDichVuRepository extends JpaRepository<ChiTietDangKyDichVu, String> {
       ...
       // Query lấy chỉ dịch vụ từ hóa đơn đã thanh toán và chưa hủy
       @Query("SELECT ct FROM ChiTietDangKyDichVu ct WHERE ct.hoaDon.khachHang.maKH = :maKH AND ct.hoaDon.trangThai = 'DaThanhToan' AND (ct.daHuy = 0 OR ct.daHuy IS NULL) ORDER BY ct.ngayBD DESC")
       ...
       // Query hủy dịch vụ
       @Query("UPDATE ChiTietDangKyDichVu ct SET ct.daHuy = 1 WHERE ct.maCTDK = :maCTDK")
       @org.springframework.data.jpa.repository.Modifying
       @org.springframework.transaction.annotation.Transactional
       void cancelService(@Param("maCTDK") String maCTDK);
}
